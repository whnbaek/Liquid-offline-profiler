...
class COCOPipeline:
    def __init__(self, file_root, ann_file, batch_size, shuffle, num_threads, device_id, world_size, cache_size):
        super().__init__()
        ...
        with self.pipe:
            jpgs, boxes, labels, img_ids = dali.fn.readers.coco(
                ...
                pad_last_batch=True, ltrb=True, tensor_init_bytes=0, cache_size=cache_size,
...
class DALICOCODataLoader:
    def __init__(self, file_root, ann_file, nsamples, batch_size=1, drop_last=False, collate_fn=None, 
                 shuffle=False, num_threads=4, device_id=0, world_size=1, cache_size=1):
        ...
        self.pipe = COCOPipeline(file_root, ann_file, batch_size, shuffle, num_threads, device_id, world_size, cache_size)
    ...
    def preprocess(self, pipe_out):
        ...
        for i in range(len(image_list)):
            img = torch.empty(image_list[i].shape(), device="cuda", dtype=torch.float)
