...
def main(args):
    ...
    ann_file = os.path.join(args.path, 'citypersons.json')
    imgs = COCO(ann_file).imgs
    nsamples = len(imgs)
    dataset_size = 0
    for v in imgs.values():
        dataset_size += os.path.getsize(os.path.join(args.path, v['file_name']))

    rank = yolo.get_rank()
    if rank == 0:
        print('env: CoorDL, model: YOLO')
    buffer_ratio = args.buffer_size / (dataset_size // 1024)
    cache_size = int(nsamples * buffer_ratio / args.world_size)
    ...
    d_train = yolo.DALICOCODataLoader(
        args.path, ann_file, nsamples, batch_size=args.batch_size, collate_fn=yolo.collate_wrapper,
        drop_last=True, shuffle=True, device_id=args.gpu, world_size=args.world_size,
        num_threads=args.num_threads, cache_size=cache_size)
    ...
    num_classes = 6  # len(d_train.dataset.classes)
    model = yolo.YOLOv5(num_classes, model_sizes[args.model_size], img_sizes=args.img_sizes).cuda()  # .to(device)
    ...
    # for epoch in range(start_epoch, args.epochs):
        ...
        # A = time.time()
    # warm up
    args.lr_epoch = lr_lambda(0) * args.lr
        # print("lr_epoch: {:.4f}, factor: {:.4f}".format(args.lr_epoch, lr_lambda(epoch)))
    yolo.train_one_epoch(model, optimizer, d_train, args, scaler)
    if args.distributed:
        dist.barrier()
    if rank == 0:
        t = time.perf_counter()
...
if __name__ == "__main__":
    ...
    parser.add_argument("--path", default="/data/coco2017") # root directory of the dataset
