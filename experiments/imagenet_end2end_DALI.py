...
@pipeline_def
def create_dali_pipeline(file_root, num_shards, shard_id):
    images, labels = fn.readers.file(file_root=file_root, num_shards=num_shards, pad_last_batch=True, preserve=True,
                                     shard_id=shard_id, shuffle_after_epoch=True, tensor_init_bytes=0,
                                     name='Reader')
...
def main():
    ...
    if args.local_rank == 0:
        print('env: DALI, model: {}'.format(args.arch))
    ...
    pipe = create_dali_pipeline(batch_size=args.batch_size, num_threads=args.workers,
                                ...
                                num_shards=args.world_size, shard_id=args.local_rank)
    pipe.build()
