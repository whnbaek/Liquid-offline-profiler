...
class COCOPipeline:
    def __init__(self, paths, index_paths, batch_size, shuffle, num_threads, device_id, world_size, initial_fill):
        super().__init__()
        ...
        with self.pipe:
            ...
            # )
            boxes, jpgs, img_ids, labels = dali.fn.readers.webdataset(
                ext=['bbox', 'image', 'image_id', 'label'], paths=paths,
                dtypes=[dali.types.DALIDataType.FLOAT, dali.types.DALIDataType.UINT8,
                        dali.types.DALIDataType.INT32, dali.types.DALIDataType.INT32],
                index_paths=index_paths, initial_fill=initial_fill,
                missing_component_behavior='error', num_shards=world_size,
                pad_last_batch=True, random_shuffle=shuffle, shard_id=device_id,
                tensor_init_bytes=0, stick_to_shard=True)
            # images = dali.fn.decoders.image(jpgs, device="mixed")
            images = dali.fn.decoders.image(jpgs, device="mixed",
                device_memory_padding=0, host_memory_padding=0, hybrid_huffman_threshold=0)
            images = dali.fn.crop_mirror_normalize(images, std=255.)
            boxes = dali.fn.reshape(dali.fn.expand_dims(boxes, axes=0), shape=[-1, 4])
...
class DALICOCODataLoader:
    def __init__(self, paths, index_paths, nsamples, batch_size=1, drop_last=False, collate_fn=None,
                 shuffle=False, num_threads=4, device_id=0, world_size=1, initial_fill=1):
        # self.dataset = COCODataset(file_root, ann_file, train=True)
        shard, extra = nsamples // world_size, nsamples % world_size
        ...
        self.pipe = COCOPipeline(paths, index_paths, batch_size, shuffle, num_threads, device_id, world_size, initial_fill)
    ...
    def preprocess(self, pipe_out):
        ...
        for i in range(len(image_list)):
            img = torch.empty(image_list[i].shape(), device="cuda", dtype=torch.float32)
            feed_ndarray(image_list[i], img)
