...
def main(args):
    ...
    paths.sort()
    index_paths = [x.replace('.tar', '.idx') for x in paths]
    nsamples = 0
    ...
    d_train = yolo.DALICOCODataLoader(
        paths, index_paths, nsamples, batch_size=args.batch_size, collate_fn=yolo.collate_wrapper,
        drop_last=True, shuffle=True, device_id=args.gpu, world_size=args.world_size,
        num_threads=args.num_threads, initial_fill=initial_fill)
