...
@pipeline_def
def _get_imagenet_load_pipe(file_root: str, num_shards: int, shard_id: int):
...
@pipeline_def
def _get_imagenet_decode_pipe(file_root: str, num_shards: int, shard_id: int,
                             write_desc: int):
...
@pipeline_def
def _get_cityscapes_decode_pipe(img_files: List[str], lbl_files: List[str], num_shards: int, shard_id: int, write_desc: int):
...
def DALI_profile_load(device_id: int, batch_size: int, num_threads: int, num_shards: int,
    ...
    if kind == 'imagenet':
        ...
        load_pipe = _get_imagenet_load_pipe(batch_size=batch_size, num_threads=num_threads, device_id=device_id,
                                            file_root=dir_path, num_shards=num_shards, shard_id=device_id)
    else:  # cityscapes
        img_files = []
        lbl_files = []
        ...
        load_pipe = _get_cityscapes_load_pipe(batch_size=batch_size, num_threads=num_threads, device_id=device_id,
                                              img_files=img_files, lbl_files=lbl_files, num_shards=num_shards, shard_id=device_id)
...
def DALI_profile_prep(device_id: int, batch_size: int, num_threads: int,
    ...
    if kind == 'imagenet':
        ...
        decode_pipe = _get_imagenet_decode_pipe(batch_size=batch_size, num_threads=num_threads, device_id=device_id,
                                                file_root=dir_path, num_shards=num_shards, shard_id=device_id, write_desc=wd)
    else:  # cityscapes
        img_files = []
        lbl_files = []
        for subdir in dir_paths:
        ...
        decode_pipe = _get_cityscapes_decode_pipe(batch_size=batch_size, num_threads=num_threads, device_id=device_id,
                                                  img_files=img_files, lbl_files=lbl_files, num_shards=num_shards, shard_id=device_id, write_desc=wd)
